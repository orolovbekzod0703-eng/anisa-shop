...
-- STORAGE
-- ============================================================
-- Public bucket "anisa" — rasmlar uchun
insert into storage.buckets (id, name, public)
values ('anisa', 'anisa', true)
on conflict (id) do nothing;

drop policy if exists "public read anisa"  on storage.objects;
drop policy if exists "admin write anisa"  on storage.objects;
drop policy if exists "admin update anisa" on storage.objects;
drop policy if exists "admin delete anisa" on storage.objects;

create policy "public read anisa"  on storage.objects for select
  using (bucket_id = 'anisa');
create policy "admin write anisa"  on storage.objects for insert
  with check (bucket_id = 'anisa' and public.is_admin());
create policy "admin update anisa" on storage.objects for update
  using (bucket_id = 'anisa' and public.is_admin());
create policy "admin delete anisa" on storage.objects for delete
  using (bucket_id = 'anisa' and public.is_admin());
