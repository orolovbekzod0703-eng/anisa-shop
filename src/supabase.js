...
export async function submitOrder(order) {
  if (!hasSupabase) {
    console.warn('[Anisa] Supabase not configured. Order saved to localStorage only.')
    const local = JSON.parse(localStorage.getItem('anisa_orders') || '[]')
    const rec = { ...order, id: `LOCAL-${Date.now()}`, status: 'new', created_at: new Date().toISOString() }
    local.push(rec)
    localStorage.setItem('anisa_orders', JSON.stringify(local))
    return rec.id
  }
...
export async function uploadProductImage(file) {
  ...
  const { error } = await supabase.storage.from('anisa').upload(path, file, { cacheControl: '3600', upsert: false })
  if (error) throw error
  const { data } = supabase.storage.from('anisa').getPublicUrl(path)
